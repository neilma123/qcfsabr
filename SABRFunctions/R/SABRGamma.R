...
SABRGamma <- function(F0, K, tex, rfr, Alpha, Beta, Rho, Nu){
  ...
  d1 <- (log(F0 / K) + 0.5 * SABRImpVol ^ 2  * tex) / (SABRImpVol * sqrt(tex))

  #Calculate Black-76 Gamma:
  Black76GammaPart <- SABRFunctions::Black76Gamma(F0, K, SABRImpVol, tex, rfr)

  #Calculate Black-76 Vega:
  Black76VegaPart <- SABRFunctions::Black76Vega(F0, K, SABRImpVol, tex, rfr)
  ...
  CorrectionTerm <- exp(-rfr * tex) * stats::dnorm(d1) - K * exp(-rfr * tex) * stats::dnorm(d1)
