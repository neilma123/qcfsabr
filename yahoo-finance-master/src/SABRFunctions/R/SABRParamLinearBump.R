...
SABRParamLinearBump <- function(F0, K, tex, Alpha, Beta, Rho, Nu, bump_param, bumpsize = 1 / 20000, bumpdir = 'up'){
  if (bump_param %in% c('F0', 'Alpha', 'Beta', 'Rho', 'Nu')){
  }
  else stop("Bump parameter MUST be one of 'F0', 'Alpha', 'Beta', 'Rho', or 'Nu'!")

  if (bumpdir %in% c('up', 'dn')){
    bumpmult <- ifelse(bumpdir == 'up', 1, -1)
  }
  else stop("Bump direction MUST be 'up' or 'dn'!")

  #Calculate the bump:
  bump <- bumpmult * bumpsize

  #Switch statement to figure out which actual parameter to bump:
  BumpedSABRVol <- switch(bump_param,
                          'F0'    = SABRFunctions::SABRtoBlack76(F0 + bump, K, tex, Alpha, Beta, Rho, Nu),
                          'Alpha' = SABRFunctions::SABRtoBlack76(F0, K, tex, Alpha + bump, Beta, Rho, Nu),
                          'Beta'  = SABRFunctions::SABRtoBlack76(F0, K, tex, Alpha, Beta + bump, Rho, Nu),
                          'Rho'   = SABRFunctions::SABRtoBlack76(F0, K, tex, Alpha, Beta, Rho + bump, Nu),
                          'Nu'    = SABRFunctions::SABRtoBlack76(F0, K, tex, Alpha, Beta, Rho, Nu + bump)
