...
#'  Nu = 1.0504)
#'
SABRtoBlack76 <- function(F0, K, tex, Alpha, Beta, Rho, Nu){
  #Setup a series of coefficients that will then be multiplied together:
  k1 <- (F0 * K) ^ ((1 - Beta) / 2)
  k2 <- 1 + (1 - Beta)^2 / 24 * (log(F0 / K)) ^2 + (1 - Beta)^4 / 1920 * log(F0 / K) ^ 4
  ...
  if(F0 != K){return((Alpha / (k1 * k2)) * (z / xz) * (1 + tex *(k3 + k4 + k5)))}
  else return((Alpha / (k1 * k2) * 1 * (1 + tex *(k3 + k4 + k5))))
}
